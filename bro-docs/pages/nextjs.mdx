...
3. **Auto-Generated Docs (OpenAPI / Scalar)**: The adapter does not currently generate the interactive `/docs` UI.
4. **Graceful Shutdown**: Serverless functions don't have a "shutdown" phase, so the `onShutdown` hook is ignored.

If you need WebSockets, cron jobs, or auto-generated documentation, you should use the standard `bro.js` framework by running `npx bro init` or `npx create-bro-framework@latest` in a new folder!
